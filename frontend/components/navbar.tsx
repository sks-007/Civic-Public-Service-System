...
export function Navbar() {
  ...
  useEffect(() => {
    const user = localStorage.getItem("cpss-user")
    setIsLoggedIn(!!user)
  }, [pathname])
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-gradient-to-r from-background/90 via-background/80 to-background/90 backdrop-blur-md">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          {isLoggedIn ? (
            <Button size="sm" className="hidden gap-1.5 sm:flex" variant="outline" onClick={handleLogout}>
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-80 p-0">
              ...
              <div className="flex flex-col gap-1 p-4">
                ...
                {isLoggedIn ? (
                  <Button className="w-full gap-2" variant="outline" onClick={() => { handleLogout(); setOpen(false); }}>
                    <LogOut className="h-4 w-4" />
                    Log Out
                  </Button>
                ) : (
